...
public class EnemyInteraction : MonoBehaviour
{
    ...
    private void EnemyGetHit(Collider other)
    {
        if (other.CompareTag("PlayerProjectile"))
        {
            ...
            if (currentHp == 0)
            {
                ...
                audioManager.PlayGothicDeathSound();
    ...
    {
        if (collision.collider.CompareTag("IAmDeath") && collision.collider.GetComponent<Rigidbody>().velocity.magnitude > 2f)
        {
            currentHp =  0;
            if (gazedAt)
            {
